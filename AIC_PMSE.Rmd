...
<!-- based on Kuchibhotla et al. (2022) -->
...
「正しい」モデル(model1)を選んだ割合は`r round(summary(factor(min_aic))[1] / R * 100, 0)`%
...
「正しい」モデル(model1)を選んだ割合は`r round(summary(factor(min_aicc))[1] / R * 100, 0)`%

## BIC {#ex1_BIC}

BICは、候補に「正しい」モデル（データを生成したモデル）を含んでいて、
サンプルサイズがじゅうぶんに大きければ、「正しい」モデルを選択する。
...
「正しい」モデル(model1)を選んだ割合は`r round(summary(factor(min_bic))[1] / R * 100, 0)`%

この場合、サンプルサイズが大きくなれば、「正しい」モデルを選ぶようになるが...
...
N \left( \begin{pmatrix} 0 \\ 0 \end{pmatrix}, \begin{pmatrix} 1 & 1 \\ 1 & 9 \end{pmatrix} \right)
$$

$$
y \sim N(x_1 + 0.3 x_2, 4^2)
$$
